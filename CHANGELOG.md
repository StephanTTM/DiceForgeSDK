...
- **The web presenters play rerolls and explosions as stories too** — parity with the Godot presenter. `visualDiceForEvent` now returns the settled stage (ADR-0016): values lost to rerolls collapse into the die that replaced them (carried as `rerolledFaces`), and explosion-born dice take their own seats (`exploded` and `bornOf` mark the lineage), so what is on the table always sums to the record's total — a rerolled value no longer lingers looking like a dropped die. The WebGL renderer plays the story with authored motion: land on the doomed value, hold, re-toss; celebrate an explosion with a face-preserving hop-and-turn while the earned die drops in mid-celebration. The physics presenter simulates each chapter as its own real throw in the same fixed tray — the re-toss and the earned die's drop-in are honest recordings, seeded reproducibly, and their collisions join the knock schedule at their place on the story's clock. Tiles show the settled stage. Both story walkers are pure modules with headless tests, the React demo gains one-click story buttons, and three visual regression scenes pin the settled stages. Plain rolls simulate and draw exactly as before.
- **DiceForge for Godot** (ADR-0021): the headless engine as a Godot 4 addon, in GDScript — `DiceForge.seeded("table-42").roll("2d20kh1+3")` produces the same schema v2 record as the TypeScript core, bit for bit. Grammar v1.2 in full, custom dice, coin flips, position-exact notation errors. Held to new **conformance vectors** exported from the core (`packages/testing/vectors/core-vectors.json`, shipped with `@diceforge-sdk/testing`): seeded RNG streams, parses, error positions and resolved records that every port must reproduce exactly. A freshness test regenerates the vectors on every run so they cannot drift from the core, and the addon's conformance scene passes 57/57 in Godot 4.7.1. Engine only — Godot presentation is future work, as is the Unity C# port against the same vectors.
- **Godot rerolls and explosions are played as stories**: the record's rolled order reconstructs each die's history, so a rerolled die lands, holds its doomed value, re-tosses and lands its successor, while a die that explodes keeps its seat and celebrates — a hop with a full vertical turn and a swell, provably face-preserving — as the bonus die it earned drops in beside it; chains repeat, each new die celebrating in turn, and per-die timelines run in parallel. The settled stage holds every die that exists at the story's end, so the faces always sum to the record's total. `roll_stage(record)` exposes the reconstruction; every settled face is mechanically checked against it, and the demo plays a double-explosion chain and a two-reroll story. (An earlier cut destroyed exploded dice, which broke the visual sum — revised on the product owner's read of the mechanic before any release carried it.)
- **Godot throw feel and scatter**, from the product owner's second session: the fall read as dice on the moon — measured at a twentieth of real gravity — and is retuned to ~4x slow motion of the real thing, with bounce sizes following a restitution of 0.35 and tumble that stays energetic until just before rest. And `presenter.scatter = true` strews resting spots collision-free with random headings instead of the reading grid; a heading about the vertical cannot change which face is up, so the record checks pass unchanged. Authored flights cross and jostle, but true rigid-body dice contact still waits on engine physics stepping.
- **Godot presenter fixes from first real use**: a dropped die flew already-dimmed and then dimmed again at the reveal — animated layouts now build undimmed and dim exactly once, asserted equal to the posed presentation's scale and albedo. And presenting while a roll was still in flight crashed against freed dice; presentations now carry a generation counter, so a superseded animation stands down at its next frame and returns `false` — rerolling mid-flight is a supported move, covered by a demo check that reproduces the report.
- **Godot bundle**: `npm run godot:bundle` composes the addon and the forge dice into the Asset Library layout — one download, zero configuration: `configure()` with no arguments finds the dice bundled beside the scripts, resolved relative to the script itself so a relocated install still works. Inside a project the presenter now prefers editor-imported resources (`load()`), which is what makes exported games work with no export-filter tweaks; external directories still runtime-load. A `Godot bundle` workflow publishes the layout as an artifact and as the orphan `godot-asset` branch, which is the shape an Asset Library listing consumes. Verified by unzipping into a fresh project and rolling the golden-vector record with no setup.
- **Godot rolling motion**: `present_animated(record)` rolls the dice in — drop, bounce, and free tumble easing into exactly the calibrated pose, with dropped dice dimming once the roll has landed. An authored animation in ADR-0007's sense: designed to end on the recorded face, never a simulation corrected into one. Godot exposes no manual physics stepping to scripts (measured by `tests/capability.gd`), so the web's record-then-replay physics (ADR-0018) is not portable to GDScript today; the demo captures the tumble mid-air and settled, and mechanically re-verifies every settled face against the record.
