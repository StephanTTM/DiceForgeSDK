...
- **Godot rerolls and explosions are played as stories**: the record's rolled order reconstructs each die's lineage, so a rerolled die lands, holds its doomed value, re-tosses and lands its successor, while an exploding die holds its highest face, pops out of existence, and its successor drops onto the same spot — chains repeat, per-die timelines run in parallel, and both the animated and posed presentations end on one die per lineage. Every settled face is still mechanically checked against the record; the demo plays a double-explosion chain and a two-reroll story. Destroyed values still count toward the record's total, which the stage alone therefore no longer shows — the README says to print `total` beside the dice.
